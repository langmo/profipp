   ...
   void * arg)
{
   ...
   {
      int errsv = errno; 
      char* message = strerror(errsv);
      LOG_FATAL(PF_PNAL_LOG, "Could not initialize Profinet socket: %s\n", message);
      if(errsv == EPERM)
         LOG_FATAL(PF_PNAL_LOG, "Likely permissions are missing. Are you running as root?");
   }
